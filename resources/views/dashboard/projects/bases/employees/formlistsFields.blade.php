        ...
        <table class="table table-sm table-bordered">
            <thead class="">
                <tr>
                    ...
                    <th class="border border-dark text-center text-uppercase" colspan="3" style="font-size: 2em;">{{ $formlist->description }}
                    </th>
                    <th class="border border-dark text-center" colspan="2" rowspan="2">
                        ...
                        src='{{ asset("{$employee->user->image_path}") }}'
                        @else
                    ...
                    @foreach ($fields as $key => $field)
                        <tr>
                            <td class="border text-center border-dark p-0 m-0">
                                ...
                                <div class="dropdown" id="dropdownMenu{{$key}}" with="5px" style="max-height: 10px;"
                                    padding="0" margin="0">
                                    ...
                                    <div class="dropdown-menu" aria-labelledby="dropdownMenu{{ $key }}">
                                        ...
                                        @if (!$field->date_returned)
                                            <button class="dropdown-item bg-info"
                                                onclick="newDevolution({{ $field->id }},{{ $field->stoks->sector_id }})"
                                                type="button">Devolução</button>
                                            <button class="dropdown-item bg-success"
                                                onclick="replacementField({{ $field->stok_id }},{{ $field->id }},{{ $formlist_employee->id }},{{ $field->qtd_delivered }})"
...
@section('js')
    ...
    <script>
        ...
        function replacementField(stok_id, field_id, formlist_id, qtd_delivered) {
            $.ajaxSetup({
                headers: {
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                }
    ...
    //                 </div>`,
        ...
        //     })
        // })()
        function newDevolution(field_id, sector_id) {
            console.log(field_id, sector_id)
            let steps = [{
