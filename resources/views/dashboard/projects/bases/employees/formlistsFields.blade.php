                            ...
                            <td class="border border-dark text-center p-0">
                                <p style=" margin: 0; font-size: 1em;">
                                    ...
                                    @if ($field->observation)
                                        <i style="float: left; margin-left:5px; margin-top:2px; font-size: 1.3em;" data-info="{{$key}}" class="fa fa-info-circle text-info info" aria-hidden="true"></i>
                                    @endif
                                    ...
                                    @if ($field->stoks->documents()->exists())
                                        <a href="{{ route('dashboard.bases.employees.formlists.documents', [$base, $employee, $formlist_employee, $field->stoks]) }}"
                                            style="float: right; margin-right:5px" target="_blank"><i class="fa fa-book fa-lg text-danger fa-lg ml-2 mt-1"
                            ...
                            <td class="border border-dark text-center">
                                <p style="padding: 0; margin: 0; font-size: 0.8em;">
                                    @if ($field->date_delivered && $field->signature_delivered)
                                        <a href="{{URL::signedRoute('extern.field.showSignature',['signatureField' => $field->signature_delivered,'field' => $field,],null,true)}}" class="text-dark font-weight-bold" target="_blank" rel="noopener noreferrer">Assinatura digital</a>
                                    @else
                                        Falha na assinatura
                                    @endif
                                </p>
                            ...
                            <td class="border border-dark text-center">
                                <p style="padding: 0; margin: 0; font-size: 0.8em;">
                                    @if ($field->date_returned)
                                        ...
                                        <a href="{{URL::signedRoute('extern.field.showSignature',['signatureField' => $field->signature_returned,'field' => $field,],null,true)}}" class="text-dark font-weight-bold" target="_blank" rel="noopener noreferrer">Assinatura digital</a>
                                        @else
                                            Falha na assinatura
                                        @endif
                                    @endif
    ...
    <script>
        ...
        async function loweringAndAdd(params) {
            ...
            if (data_replacements) {
                data_replacements.location = JSON.parse(localStorage.geolocation).full
                Swal.fire({
...
@section('css')
    ...
    <style>
        #swal2-input, #user_pass, #employee_pass {
