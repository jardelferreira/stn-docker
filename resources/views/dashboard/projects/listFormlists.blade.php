...
@section('content_header')
    <div class="form-check form-check-inline">
        ...
        <button type="button" class="btn btn-success btn-sm ml-2"
            onclick="downloadZipFormlists('{{ route('api.dashboard.downloadManyFormlistsPdf') }}')">Baixar fichas
            selecionadas (.zip)</button>
    </div>
    <hr>
@stop
@section('css')
                ...
                @foreach ($bases as $base)
                    @foreach ($base->employees as $employee)
                        @foreach ($employee->formlistsFromEmployee as $formlistsFromEmployee)
                            <tr>
                                <td scope="row">{{ $formlistsFromEmployee->id }}</td>
                                <td scope="row" class="text-center">
                                    <label class="custom-checkbox ml-4">
                                        <input type="checkbox" id="checkbox-{{ $formlistsFromEmployee->id }}"
                                            name="formlists[]" value="{{ $formlistsFromEmployee->id }}">
                                        <span class="checkmark"></span>
                                    </label>
                                </td>
