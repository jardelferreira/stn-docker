...
@section('content_header')
    <h1>Vincular, <span class="font-weight-bold">{{ $document->name }}</span> ao estoque do setor - <span
...
@section('content')
    @if ($stoks->count())
        <form action="{{route('dashboard.documents.attachDocumentToStoks',$document)}}" method="POST">
            @csrf
            @method("POST")
            <h4>Selecione os item que deseja vincular e clique no botão <button type="submit" class="btn btn-primary">Vincular
                    marcados</button></h4>
            <hr>
