    ...
    <form action="{{route('dashboard.invoices.store')}}" method="post" autocomplete="off" enctype="multipart/form-data">
        ...
        <div class="form-group col-lg-6 col-sm-12 col-md-12">
          ...
          <select id="departament_cost" class="form-control" name="departament_cost_id">
            ...
            @foreach ($departament_costs as $item)
                <option value="{{$item->id}}"><small>{{$item->sectorCost->cost->project->name}} => {{$item->sectorCost->cost->name}} => {{$item->sectorCost->name}} => {{$item->name}}</small></option>
            @endforeach
          </select>
      ...
      <div class="form-row">
        <div class="form-group col-lg-3 col-md-6 col-sm-12">
          ...
          <input type="text" step="0.01" autocomplete="off" class="form-control" name="value" id="value" aria-describedby="helpName" placeholder="R$ 1.000,00">
        ...
        <div class="form-group col-lg-3 col-md-6 col-sm-12">
          ...
          <input type="text" step="0.01" autocomplete="off" class="form-control" name="value_departament" id="value_departament" aria-describedby="helpName" placeholder="R$ 1.000,00">
          <small id="helpName" class="form-text text-muted">Informar valor</small>
        </div>
...
<script>
  ...
  $("#value, #value_departament").inputmask( 'currency',{"autoUnmask": true,
            radixPoint:",",
            groupSeparator: ".",
            allowMinus: false,
            prefix: 'R$ ',            
